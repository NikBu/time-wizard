...
function _applyChecklistSettings() {
  ...
  const pdMap = {
    ...
    normal: 'var(--space-3)',
    relaxed: 'var(--space-4) var(--space-5)'
  };
...
function _renderSettingsPanel() {
  ...
  el.innerHTML = `
    ...
    <div class="cl-settings-grid">
      ...
      <div class="cl-settings-row">
        <label class="cl-settings-label">Drag horizontally to indent/dedent</label>
        <label class="cl-toggle-wrap">
          <input type="checkbox" ${checklistSettings.dragIndent?'checked':''} onchange="_clSetSetting('dragIndent',this.checked)">
          <span class="cl-toggle"></span>
        </label>
      </div>
    </div>`;
...
function renderChecklist(){
  ...
  main.innerHTML=`
    ...
    <ul class="task-tree" id="taskTree">${_renderNodes(list.tasks,null)}</ul>
...
function _renderNodes(tasks, pid){
  ...
  return siblings.map((t, idx) => {
    ...
    const childrenHtml=(!isCollapsed&&hasChildren)
      ? `<ul class="task-tree task-tree--nested">${_renderNodes(tasks,t.id)}</ul>`
      : '';
...
function _pdStart(e) {
  ...
  _pd.active  = true;
  _pd.taskId  = taskId;
  _pd.startX  = e.clientX;
  _pd.startY  = e.clientY;
...
function _pdMove(e) {
  ...
  if (checklistSettings.dragIndent) {
    ...
    else if (dx < -INDENT_THRESHOLD) newHint = 'dedent';
  }
  if (newHint !== _pd.indentHint) {
    _pd.indentHint = newHint;
